...
void setup_cpu(int cpu) {
  ...
  if (ret || (num_fds == 0)) errx(1, "cannot setup events\n");
...
int initialize() {
  ...
  options.events =
      "instructions,cycles,DTLB_LOAD_MISSES:MISS_CAUSES_A_WALK,iTLB-load-"
      "misses,MISPREDICTED_BRANCH_RETIRED,L1-icache-load-misses,MEM_LOAD_"
      "RETIRED:L1_HIT,MEM_LOAD_RETIRED:L2_HIT,MEM_LOAD_RETIRED:L3_HIT,LLC_"
      "MISSES";
  // TODO: The list of the events to measure
  ...
  for (c = 0; c < ncpus; c++) {
    for (i = 0; i < num_fds[c]; i++) {
      ...
      if (ret) err(1, "cannot enable event %s\n", all_fds[c][i].name);
    }
  }
...
void measure(perf_event_desc_t **all_fds, int *num_fds, int ncpus, int period,
  ...
  while (period--) {
    ...
    if (result_after < result_before) {
      result_delta =
          (int64_t)result_after + (int64_t)(MAX_ENERGY_COUNT - result_before);
    } else {
