...
void schedule (perf_event_desc_t **all_fds, int *num_fds, int ncpus, double pmu_matrix[][10]) {
  ...
  for (i = 0; i < CPU_NUM; i++) {
    for (j = 0; j < CPU_NUM; j++) {
      ...
      if (proc_list[i].affinity % NUMBER_OF_CORES == proc_list[j].affinity % NUMBER_OF_CORES) {
        // call smt regression
        benefit_matrix[i][j] = regression_smt (
                pmu_matrix[left][2] / pmu_matrix[left][1],
                ...
                pmu_matrix[right][9] / pmu_matrix[right][1]) * 1.04;
      }
      else {
        // call cmp regression
        benefit_matrix[i][j] = regression_cmp (
                pmu_matrix[left][2] / pmu_matrix[left][1],
                pmu_matrix[left][3] / pmu_matrix[left][1],
                pmu_matrix[left][4] / pmu_matrix[left][1],
                pmu_matrix[left][5] / pmu_matrix[left][1],
  ...
  */
  ...
  for (i = 0; i < 105; i++) {
    ...
    if (left < num_proc && right < num_proc) power += 7.3;
    else if (left < num_proc || right < num_proc) power += 6.0;
